...
public class FlintlockItem extends RangedWeaponItem {
    ...
    public FlintlockItem(Item.Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (FlintlockItem.isCharged(itemStack)) {
            boolean bl = user.getAbilities().creativeMode;
            FlintlockItem.shoot(world, user, hand, user.getStackInHand(hand), itemStack, 1f, bl, 4f, 7.5f, 0);
            FlintlockItem.setCharged(itemStack, false);
            FlintlockItem.postShoot(world, user, user.getStackInHand(hand));
    ...
    private static float getSpeed(ItemStack stack) {
        if (FlintlockItem.hasProjectile(stack, Items.FIREWORK_ROCKET)) {
            return 1.6f;
        }
        return 3.15f;
    }
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        float f = FlintlockItem.getPullProgress(i, stack);
        if (f >= 1.0f && !FlintlockItem.isCharged(stack) && FlintlockItem.loadProjectiles(user, stack)) {
            FlintlockItem.setCharged(stack, true);
    ...
    private static boolean loadProjectiles(LivingEntity shooter, ItemStack crossbow) {
        ...
        for (int k = 0; k < j; ++k) {
            ...
            if (FlintlockItem.loadProjectile(shooter, crossbow, itemStack, k > 0, bl)) continue;
            return false;
        }
        return true;
    }
    ...
    private static boolean loadProjectile(LivingEntity shooter, ItemStack crossbow, ItemStack projectile, boolean simulated, boolean creative) {
        ...
        FlintlockItem.putProjectile(crossbow, itemStack);
        return true;
    }
    ...
    public static boolean hasProjectile(ItemStack crossbow, Item projectile) {
        return FlintlockItem.getProjectiles(crossbow).stream().anyMatch(s -> s.isOf(projectile));
    }

    private static void shoot(World world, LivingEntity shooter, Hand hand, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean creative, float speed, float divergence, float simulated) {
        ProjectileEntity projectileEntity = new MusketBallEntity(shooter, world);
        if (world.isClient) {
            return;
        }
...
//        } else {
//            projectileEntity = FlintlockItem.createArrow(world, shooter, crossbow, projectile);
...
//        crossbow.damage(bl ? 3 : 1, shooter, e -> e.sendToolBreakStatus(hand));
    ...
    public static void shootAll(World world, LivingEntity entity, Hand hand, ItemStack stack, float speed, float divergence) {
        List<ItemStack> list = FlintlockItem.getProjectiles(stack);
        float[] fs = FlintlockItem.getSoundPitches(entity.getRandom());
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (i == 0) {
                FlintlockItem.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, 0.0f);
                continue;
            }
            if (i == 1) {
                FlintlockItem.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, -10.0f);
                continue;
            }
            if (i != 2) continue;
            FlintlockItem.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, 10.0f);
        }
        FlintlockItem.postShoot(world, entity, stack);
    }

    private static float[] getSoundPitches(Random random) {
        boolean bl = random.nextBoolean();
        return new float[]{1.0f, FlintlockItem.getSoundPitch(bl, random), FlintlockItem.getSoundPitch(!bl, random)};
    }
    ...
    private static void postShoot(World world, LivingEntity entity, ItemStack stack) {
        if (entity instanceof ServerPlayerEntity serverPlayerEntity) {
            ...
            serverPlayerEntity.getItemCooldownManager().set(ModItems.FLINTLOCK, 150);
            serverPlayerEntity.incrementStat(Stats.USED.getOrCreateStat(stack.getItem()));
        }
        FlintlockItem.clearProjectiles(stack);
    }
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        if (!world.isClient) {
            ...
            float f = (float)(stack.getMaxUseTime() - remainingUseTicks) / (float)FlintlockItem.getPullTime(stack);
            if (f < 0.2f) {
                this.charged = false;
                this.loaded = false;
            }
    ...
    public int getMaxUseTime(ItemStack stack) {
        return FlintlockItem.getPullTime(stack) + 3;
    }
    ...
    private static float getPullProgress(int useTicks, ItemStack stack) {
        float f = (float)useTicks / (float)FlintlockItem.getPullTime(stack);
        if (f > 1.0f) {
            f = 1.0f;
        }
        return f;
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        List<ItemStack> list = FlintlockItem.getProjectiles(stack);
        if (!FlintlockItem.isCharged(stack) || list.isEmpty()) {
            return;
        }
